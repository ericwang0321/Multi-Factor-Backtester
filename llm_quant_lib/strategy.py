...
import pandas as pd
import numpy as np
import deepseek # 确保你已安装: pip install deepseek
import json
import os
...
class LLMStrategy(BaseStrategy):
    ...
    def __init__(self, universe_df: pd.DataFrame, universe_to_trade: str = 'equity_us', top_n: int = 5, api_key: Optional[str] = None):
        ...
        Args:
            universe_df (pd.DataFrame): 资產池定义。
            universe_to_trade (str): 指定 AI 关注哪个资產池 (例如 'equity_us')。 'All' 代表使用所有资產。
            top_n (int): 要求 AI 选择的 ETF 数量上限。
            api_key (str, optional): DeepSeek API 密鑰。如果为 None，则尝试从环境变量 DEEPSEEK_API_KEY 读取。
        """
        super().__init__(universe_df)
        self.universe_to_trade = universe_to_trade
        self.top_n = top_n
        # ---【假设 1】---
        # 假设你的 DeepSeek API Key 存储在环境变量 DEEPSEEK_API_KEY 中。
        ...
        try:
            ...
            raise RuntimeError(f"初始化 DeepSeek Client 失败，请检查 API Key 是否有效: {e}")
    ...
    def get_target_weights(self, current_date: pd.Timestamp, factor_snapshot: pd.DataFrame, portfolio_state: dict) -> Dict[str, float]:
        ...
        调用 LLM API 获取目标权重。
        """
        print(f"\n[LLMStrategy] 正在 {current_date.date()} 为资產池 '{self.universe_to_trade}' 请求 AI 决策...")
        ...
        if current_factor_data.empty:
            print(f"[LLMStrategy] 警告: 在 {current_date.date()} 未找到资產池 '{self.universe_to_trade}' 的有效因子数据。清空仓位。")
        ...
        try:
            ...
            if valid_selection:
                ...
                if target_securities:
                     ...
                     print("[LLMStrategy] AI 推荐的有效资產为空，清空仓位。")
                     target_weights = {} # 清仓
            else:
                 print("[LLMStrategy] AI 未推荐任何有效资產，清空仓位。")
                 target_weights = {} # 清仓
            ...
            print(f"[LLMStrategy] 调用 DeepSeek API 或处理时发生错误: {e}。本期跳过交易。")
            import traceback
    ...
    def _create_prompt(self, current_date: pd.Timestamp, factor_data: pd.DataFrame, portfolio_state: dict) -> List[Dict]:
        ...
        你将收到当前日期、当前持仓（仅供参考），以及资產池 '{self.universe_to_trade}' 中所有可用 ETF ({num_assets} 支) 的技术因子快照（EMA, MACD, RSI）。

        因子解释:
        - ema_12, ema_26: 短期和长期指数移动平均线。价格在均线之上通常看涨，金叉 (短期上穿长期) 是买入信号。
        - macd_line: MACD 快线。正值且上升表示强劲上升趋势。
        - macd_signal: MACD 信号线（慢线）。快线在慢线上方且距离拉大是看涨信号。
        - macd_hist: MACD 柱状图 (快线 - 慢线)。柱状图在零轴上方且放大是强烈的看涨信号。
        ...
        ### ETF 技术因子快照 (资產池: '{self.universe_to_trade}', CSV 格式)
        ```csv
        {factor_csv}
        ```
