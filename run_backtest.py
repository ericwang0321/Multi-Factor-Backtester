...
if __name__ == '__main__':
    ...
    if db_conf.get('use_db', False):
        ...
        if not DB_CONFIG.get('password') and db_conf.get('password', '').startswith('${'): # 检查是否未设置环境变量
    ...
    COMPOSITE_BENCHMARK_CONFIG = paths_conf.get('composite_benchmark', {}) # 获取综合基准配置
    BENCHMARK_DATA_PATH = None # 单个基准文件路径
    BENCHMARK_DISPLAY_NAME = "Benchmark" # 图表显示名称

    if SELECTED_UNIVERSE.lower() == 'all' and COMPOSITE_BENCHMARK_CONFIG:
        print(f"检测到 universe 为 'All'，将使用 config.yaml 中定义的综合基准。")
        # composite_config 已经加载
        BENCHMARK_DISPLAY_NAME = COMPOSITE_BENCHMARK_CONFIG.get('display_name', 'Composite Benchmark')
        # 不需要 BENCHMARK_DATA_PATH，DataHandler 会处理
    else:
        # 获取单个基准文件名
        ...
        if BENCHMARK_FILENAME:
            ...
            BENCHMARK_DISPLAY_NAME = BENCHMARK_FILENAME # 使用文件名作为默认显示名
        ...
        if not BENCHMARK_DATA_PATH:
             ...
             BENCHMARK_DISPLAY_NAME = "N/A" # 无基准时不显示
    # --- 【修改结束】---
    ...
    benchmark_df = None # 初始化 benchmark_df
    try:
        ...
        # --- 【修改】根据 universe 调用不同的基准加载方法 ---
        if SELECTED_UNIVERSE.lower() == 'all' and COMPOSITE_BENCHMARK_CONFIG:
             benchmark_df = data_handler.load_composite_benchmark_data(COMPOSITE_BENCHMARK_CONFIG)
             if benchmark_df is None:
                  print("警告: 综合基准加载或计算失败。")
        elif BENCHMARK_DATA_PATH: # 如果是单个基准且路径有效
            benchmark_df = data_handler.load_benchmark_data(BENCHMARK_DATA_PATH)
            if benchmark_df is None:
                 print(f"警告: 单个基准文件 '{BENCHMARK_DATA_PATH}' 加载失败。")
        # 如果 benchmark_df 仍然是 None (无配置或加载失败)，后续流程会处理
        # --- 【修改结束】---
